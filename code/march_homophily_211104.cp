/**************************************************************
 * filename : codeB_20190830
 * edited by: Yeseul Choi 
 * coded by : Minyoung Choi
 * date : 2020.07.08
**************************************************************/
...
double avg_org_perf, avg_indv_perf, avg_equilibrium, avg_num_scale, avg_group_equilibrium;
double final_avg_org_perf, final_avg_indv_perf, final_avg_equilibrium, final_zero_indv, dim_div;
/**************************************************************/
...
/******initializing MPI*******/
	...
	readDataFromFile();
	clear();
	out_to_csv.open("new_result.csv");

	for (int i = 0; i < num_sample; i++) {
		std::cout << " num sample = " << i << std::endl;
		org_initialize();
		indv_initialize();
		...
		for (int count_int = 0; count_int < num_steps; count_int++) {
			...
			sum_output(i, count_int);
			update_time(count_int);
			//indiv_code_time(i, count_int);
		} // end of iteration
		output(i);
	} // end of sample
	out_to_csv.close();
...
void knowledge_div(){
	vector<vector<int>> num_knowledges = vector<vector<int>>(num_dimensions, {0, 0, 0});
	//vector<double> dim_prob = vector<double>num_dimensions;
    double dim_div = 0.0;
    double dim_prob = 0.0;

    for (int j = 0; j < num_population; ++j)
	    for (int i = 0; i < num_dimensions; ++i)
		    ++num_knowledges[i][agent[j].indv_code[i] + 1];
	
	for (int i = 0; i< num_dimensions; i++){
		double denom = (num_population - num_knowledges[i][1]);
			if (denom > 0){
				dim_prob = num_knowledges[i][2]/denom;
				if (dim_prob <= 0.5){
					dim_div += 2*(dim_prob);
					}
				else{
					dim_div += -2*(dim_prob) + 2;
				}
			}
		}
	dim_div = dim_div/num_dimensions;
}
...
void update_indv() {  // get (1) individual performance, (2) individual fitness, (3) individual satisfaction, and (4) equilibrium knowledge level
	...
	for (int j = 0; j < num_population; j++) {
		...
		agent[j].group_homophily /= num_population;
...
void sum_output(int sample, int step) {
zero_indv = 0;
double temp_avg_group_equilibrium = 0;
...
double temp_avg_indv_perf = 0;
	...
	ss << sample << "," << step << "," <<p1 << "," << p2 << "," <<dim_div << "," << avg_equilibrium << "," << avg_org_perf << "," << avg_indv_perf << "\n";
	out_to_csv.write(ss.str().c_str(), ss.str().size());
}

void update_time(int temp_t){
	if ((temp_t%10) == 0){
		out1 << temp_t << "\t" << p1 << "\t" << p2 << "\t" << beta << "\t" << zero_indv << "\t" << avg_equilibrium << "\t" << avg_org_perf << "\t" << avg_indv_perf << "\t" << dim_div << "\t" << endl;
	}
}

void indiv_code_time(int sample, int step) {
	for(int j = 0; j <= num_population; j++){
	        stringstream st;
		st << sample << ',' << step << ',' << j;
		for (auto & x : (agent+j)->indv_code)
			st << ',' << x;
		    //out << (agent+j)->indv_code << ' ' 
		out_to_csv << st.str();
	}
}
...
//ofstream out("results.txt");
	...
	if (temp_s == num_sample - 1) {
		out << p1 << "\t" << p2 << "\t" << beta << "\t" << final_zero_indv << "\t" << final_avg_equilibrium << "\t" << final_avg_org_perf << "\t" << final_avg_indv_perf << "\t" << avg_num_scale << "\t" << dim_div << "\t" << endl;
	}
}
			/*******:wq*****************************************/
