...
void update_indv() {  // get (1) individual performance, (2) individual fitness, (3) individual satisfaction, and (4) equilibrium knowledge level
	...
	for (int j = 0; j < num_population; j++) {
		...
		for (int i = 0; i < num_dimensions; i++) {
			...
			agent[j].group_homophily += num_knowledges[i][agent[j].indv_code[i]+1];
		}
...
double temp_avg_indv_perf = 0;
	...
	for (int j = 0; j < num_population; j++) {
		temp_avg_group_equilibrium += agent[j].group_homophily / num_dimensions;
	...
	avg_group_equilibrium = temp_avg_group_equilibrium / num_population;
	...
	ss << sample << "," << step << "," <<p1 << "," << p2 << "," <<avg_group_equilibrium << "," << avg_equilibrium << "," << avg_org_perf << "," << avg_indv_perf << "\n";
